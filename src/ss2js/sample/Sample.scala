...
class Assert {
  ...
  /** enclose test case */
  def check[T](msg: String, expected: T = ())(body: T => T) = {
    testCount += 1
    var ok = false
    print(msg + "   ")
    try {
      val ret = body(expected)
      ok = ret == expected
      if(ok)
          print("OK")
      else
...
class Sample(val a: Int) extends Trait1 {
  val d = 1.1
  override def func1() = 1
}

class Sample2(a: Int) extends Sample(a) {
  val i = 1
  var s = "abc"
  type Int2 = Int
...
object Tests extends Assert {
  ...
  def tryCatchTest(doThrow: Boolean) {
    check("try catch finally " + doThrow) { e =>
    var a = 0
    try {
      a = 1
  ...
  def classTests() {
    ...
    assert(s.superFunc1 == 3, "super method")
    }
  }
  ...
  def classTests2() {
    ...
    check("inner and outer classes") { e =>
      val o1 = new Outer(1)
      val g1 = o1.Inner.get
      val o2 = new Outer(2)
      val g2 = o2.Inner.get
  ...
  def nestedTests() {
    ...
    val n = new Nested2
    check("nested object and method") { e =>
  ...
  def implicitTest() {
    ...
    check("implicit method", "richabc") { e =>
      "abc".richStringMethod()
    }
  }

  def functionalTests(b: Int = 2) {
    check("default param", 2) { e => b }
    def func(a: Int)(b: Int) = b
    check("multiple arg lists", 2) { e => func(1)(2) }
    def mycontrol(cont: Int => Int) = cont(3)
    check("closure", 6) { e => mycontrol { i => 3 + i } }
    
    class Cls(val a: Int) {
      def func(cls: Cls)(f: Cls => Int) = f(cls)
    }
    check("partially applied function", 2) { e =>
      val cls1 = new Cls(1)
      val cls2 = new Cls(2)
  ...
  def nativeTest() {
    ...
    check("native javascript code", "native") { e =>
      nativeFunc()
    }
  }
